...
```json
{
  ...
  "features": { "druidMonitor": false, "serverMonitor": false }
}
```

各版取值（如实声明，不做硬编码）：

| 版本 | framework | version 来源 | language | languageVersion 来源 | druidMonitor | serverMonitor |
|------|-----------|--------------|----------|----------------------|:---:|:---:|
| Java | `RuoYiConfig.getName()`（RuoYi） | `RuoYiConfig.getVersion()`（3.9.2） | java | `System.getProperty("java.version")` | true | true |
| GO | RuoYi-Vue-GO | 常量 1.0.0 | go | `runtime.Version()` 去前缀 | false | false |
| Python | `AppConfig.app_name`（RuoYi-Vue-FastApi） | `AppConfig.app_version`（1.0.0） | python | `platform.python_version()` | false | true |
